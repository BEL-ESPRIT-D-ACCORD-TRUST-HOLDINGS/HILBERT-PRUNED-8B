"""pytest entry points for semif86. Run with `pytest sm86/tests` from the repository root.

Model-free tests always run. Tokenizer and parity tests need the pinned
snapshot directory in SEMIF86_MODEL_DIR (tokenizer.json, chat_template.jinja)
...
MODEL_DIR = os.environ.get("SEMIF86_MODEL_DIR")


@pytest.fixture(scope="session")
def binary():
    subprocess.run(["make", "-C", str(SM86), "build/semif86"], check=True, capture_output=True)
    return SM86 / "build/semif86"
...
@pytest.mark.skipif(not MODEL_DIR, reason="set SEMIF86_MODEL_DIR to the pinned Qwen3.5-4B snapshot")
